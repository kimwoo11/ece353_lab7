code Kernel
  ...
  -- Woo Sik Kim (1003379953)

------------------------- StartUserProcess --------------------------------

  function StartUserProcess (i: int)
    --
    -- Create the new PCB for the process that is the first Thread so it
    -- can run a process on the process manager
    --
    var
        new_PCB: ptr to ProcessControlBlock
    ...
    new_PCB = processManager.GetANewProcess ()
    new_PCB.myThread = currentThread
    currentThread.myProcess = new_PCB

    open_file = fileManager.Open("TestProgram4")
    --open_file = fileManager.Open(i asPtrTo array of char)
    initUserPC = open_file.LoadExecutable (&(new_PCB.addrSpace))
    fileManager.Close (open_file)
    initUserStackTop = new_PCB.addrSpace.numberOfPages * PAGE_SIZE

    initSystemStackTop = (& currentThread.systemStack[SYSTEM_STACK_SIZE - 1]) asInteger

    oldIntStat = SetInterruptsTo (DISABLED)
    new_PCB.addrSpace.SetToThisPageTable ()
...
-------------------------- InitFirstProcess -------------------------------
  function InitFirstProcess ()
    --
    -- Function creates the first process spawned from main
    --
    var
        new_thread: ptr to Thread

    new_thread = threadManager.GetANewThread ()
    (*new_thread).Init("UserProgram")
    (*new_thread).Fork(StartUserProcess, 0)
  endFunction
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        --
      	...
      	threadTableLock = new Mutex
      	threadFree = new Condition

      	-- Initializations of Mutex and Condition variable
      	threadTableLock.Init()
      	threadFree.Init()

      	-- We want to initialize each thread status to UNUSED and add
      	-- each thread to the end of the free list
      ...
      method GetANewThread () returns ptr to Thread
        ...
        --
      	var threadPtr: ptr to Thread

      	threadTableLock.Lock()			 -- Obtain threadTable lock so no deadlocking occurs
      	while freeList.IsEmpty()		 -- Loop to wait for a free thread
      		threadFree.Wait(&threadTableLock)
      	endWhile

      	-- After obtaining the free thread, we set its status to JUST_CREATED
      	-- and unlock the table lock for other threadas to access
      ...
      method FreeThread (th: ptr to Thread)
        ...
        --
      	...
      	-- First, we obtain the table lock to mitigate deadlocking.
      	-- Then we set the status of the thread to be unused and
      	-- append it to the freeList. We then signal threadFree
      	-- to "wakeup" sleeping threads and release the lock

      	threadTableLock.Lock()
      	(*th).status = UNUSED
      	freeList.AddToEnd(th)
      	threadFree.Signal(&threadTableLock)
      	threadTableLock.Unlock()
        endMethod
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        --
      	...
      	-- We now want to initialize each of the PCBs to FREE and add each of the
      	-- entries into the freeList
      	for i = 0 to MAX_NUMBER_OF_PROCESSES - 1
      		processTable[i].Init()
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        --
      	...
      	(*ptrToPCB).pid = nextPid
      	nextPid = nextPid + 1
      	(*ptrToPCB).status = ACTIVE
    ...
    foundParent : bool
        ...
        for i = 0 to MAX_NUMBER_OF_PROCESSES - 1
          if processTable[i].parentsPid == p.pid && processTable[i].status == ZOMBIE
            ...
            aProcessBecameFree.Signal(&processManagerLock)
            endIf
          if processTable[i].pid == p.parentsPid && processTable[i].status == ACTIVE
            foundParent = true
            endIf
          endFor
...
-----------------------------  ProcessFinish  --------------------------
  ...
  function ProcessFinish (exitStatus: int)
    ...
    var
      oldIntStat: int
      oldProcess: ptr to ProcessControlBlock
      i: int

    currentThread.myProcess.exitStatus = exitStatus
    -- disable interrupts
    oldIntStat = SetInterruptsTo(DISABLED)
    oldProcess = currentThread.myProcess
    currentThread.myProcess.myThread = null
    currentThread.myProcess = null
    currentThread.isUserThread = false
    oldIntStat = SetInterruptsTo(oldIntStat)
    frameManager.ReturnAllFrames(&(oldProcess.addrSpace))
    processManager.TurnIntoZombie(oldProcess)
    for i=0 to MAX_FILES_PER_PROCESS-1
      if oldProcess.fileDescriptor[i] != null
        fileManager.Close(oldProcess.fileDescriptor[i])
        oldProcess.fileDescriptor[i] = null
      endIf
    endFor
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
	...
	-- Initializations
	var i, free_bit, frameAddress: int
	...
	for i = 0 to numFramesNeeded - 1
		free_bit = framesInUse.FindZeroAndSet()
		frameAddress = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (free_bit * PAGE_SIZE)
	...
	endMethod
  	...
  	for i = 0 to (*aPageTable).numberOfPages - 1
  		...
  		bitN = (frameAddress - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME) / PAGE_SIZE
  		framesInUse.ClearBit(bitN)
  	endFor
...
-----------------------------  Handle_Sys_Shutdown  ---------------------------------
  ...
  function Handle_Sys_Shutdown ()
      -- Fatal Error will stop program and simulate a shutdown
...
function Handle_Sys_Fork () returns int
  ...
  for i=0 to currentThread.myProcess.addrSpace.numberOfPages-1
     currentFrameAddr = currentThread.myProcess.addrSpace.ExtractFrameAddr(i)
     newFrameAddr = newThread.myProcess.addrSpace.ExtractFrameAddr(i)
     MemoryCopy(newFrameAddr, currentFrameAddr, PAGE_SIZE)
  ...
  newThread.Fork(Resume_Child_After_Fork, oldUserPC)
  return newPcb.pid

  endFunction

------------------- Resume_Child_After_Fork -----------------------------

function Resume_Child_After_Fork(init_user_pc : int)
  var
  ...
  initSystemStackTop = (& currentThread.systemStack[SYSTEM_STACK_SIZE-1]) asInteger
  -- go back to user thread
  BecomeUserThread(initUserStackTop, init_user_pc, initSystemStackTop)
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
    --
    -- function that handles Exec syscalls
    --
    var
    ...
    fileToStr = currentThread.myProcess.addrSpace.GetStringFromVirtual (&strBuffer, filename asInteger, MAX_STRING_SIZE)
    if (fileToStr == -1)
        return -1
    endIf
    ...
    if (open_file == null)
        return -1
    endIf
    initUserPC = open_file.LoadExecutable (&newAddrSpace)
    fileManager.Close (open_file)
    ...
    -- would not actually return here because BecomeUserThread never returns
    return 3000
    endFunction
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
      ...
      printHex (filename asInteger)      -- prints the hex value
      nl ()
      print ("filename = ")
      print (&strBuffer)
      nl ()
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
      ...
      if openFile != null || fileDescriptorIdx == -1
        return -1
      endIf
      ...
      currentThread.myProcess.fileDescriptor[fileDescriptorIdx] = openFile

      return fileDescriptorIdx
    endFunction
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
    ...
    while true
      ...
      synchReadStatus = fileManager.SynchRead(openFile, destAddr,
                                              nextPosInFile, thisChunkSize)
...
function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
    ...
    while true
      ...
      synchWriteStatus = fileManager.SynchWrite(openFile, sourceAddr, nextPosInFile, thisChunkSize)
      nextPosInFile = nextPosInFile + thisChunkSize
      writtenSoFar = writtenSoFar + thisChunkSize
      virtPage = virtPage + 1
      offset = 0
...
function Handle_Sys_Seek (fileDesc: int, newCurrentPos: int) returns int
    ...
    if newCurrentPos < -1 || newCurrentPos > openFile.fcb.sizeOfFileInBytes
      fileManager.fileManagerLock.Unlock()
      return -1
    elseIf newCurrentPos == -1
